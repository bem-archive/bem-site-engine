...
В случае, если данное поле будет отсутствовать, то дата последнего изменения будет
получена на этапе сборки данных как дата последнего коммита `*.md` файла, путь к которому указан  поле `content`. 
...
* Тип: {String}
* Наличие: обязательно, если отсутствует поле `stub` в значении `true`
* Значение по умолчанию: отсутствует
...
В случае, когда сайт сконфигурирован для работы с несколькими языками и для определенного
языка контент отсутствует, то можно показать страницу заглушку, содержащую сслылку на этот же ресурс
на другом языке. Это возможно с помощью установки данного поля в значение `true`
